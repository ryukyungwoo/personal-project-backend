...
<details>
<summary>Redis를 활용한 메시징 큐 시스템 구축</summary>
<br />
실시간 채팅 서비스를 이용할 때 WebSocket을 사용하여 메시지를 보내면 DB에 바로 저장하는 로직이 짜여 있었습니다.<br />
문제점은 바로 DB에 저장하는 특성 때문에 트래픽 문제로 사용자 경험이 저하되는 것입니다.<br />
그래서 캐싱 전략 중 Write-back 전략을 차용하여 큐에 메시지를 임시로 저장하였다가 일정 크기가 됐을 때 DB로 옮기는 방식을
고려하였습니다.<br />
Write-back 전략에 스프링 메세지 큐나 Rabbit MQ등을 사용하려 하였으나<br />
메모리 기반의 빠른 반응속도를 가진 Radis가 실시간 채팅에 더욱 적합한듯 하여 Redis를 활용한 메시징 큐 시스템을 구축하게
되었습니다.<br />
그 결과 트래픽을 1/10로 줄였습니다.<br />
<br />

</details>

<details>
<summary>주기적 데이터 업데이트 최적화</summary>
<br />
스프링을 활용하여 주기적으로 파이썬에서 주식정보를 가져오는 로직을 사용하였습니다.<br />
데이터를 가져오는 데 시간이 걸려서 이를 줄이기 위해서 다음과 같은 방법을 사용하였습니다.<br />
주기적으로 데이터를 가져올 때 주식의 Ticker를 ID로 하여 DB의 데이터와 중복성 검사를 하였습니다.<br />
그 결과 동일 데이터는 저장하지 않는 방식으로 로직 실행시간을 획기적으로 1/100로 줄였습니다.<br />
<br />
</details>

<details>
<summary>비동기 크롤링으로 성능 개선</summary>
<br />

주식 게시판 글을 크롤링하는 과정에서 Selenium을 사용하였습니다.<br />
Selenium은 동적 크롤링 라이브러리로 사용 중에는 파이썬이 다른 업무를 받지 못하는 문제가 있었습니다.<br />
그래서 asyncio를 사용하여 크롤링 과정을 비동기로 처리해 작업을 병렬화시켰습니다.<br />
그 결과 더욱더 효율적인 동시작업이 가능해졌습니다.<br />
